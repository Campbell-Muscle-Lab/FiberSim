...
def summary_figure():
    ...
    for i in range(no_of_conditions):
        ...
        # Force
        plot_index = ((hs_titin_row - 1) * no_of_cols) + i
        ax[plot_index].set_ylim([min_titin - 1000, max_titin + 1000])
...
def superpose_traces():
    ...
    for i in range(no_of_conditions):
        ...
        for file in os.listdir(condition_folder):
            ...
            ax[hs_force_row - 1].plot(d['time'], d['m_force'], '-')
            
    ofs = os.path.join(top_data_folder, 'force_overlay.png')
    print('Saving force_overlay figure to: %s' % ofs)
    fig.savefig(ofs, bbox_inches='tight')
