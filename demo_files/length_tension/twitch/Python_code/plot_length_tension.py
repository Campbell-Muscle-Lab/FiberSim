...
def plot_length_tension():
    ...
    for i in range(no_of_conditions):
        ...
        for file in os.listdir(condition_folder):
            ...
            titin_force = d['hs_1_titin_force'].max()
            extracellular_force = d['hs_1_extracellular_force'].max()
            passive_force = titin_force + extracellular_force
            total_force = d['hs_1_force'].max()
            active_force = total_force - passive_force
            
            # Store
            hsl.append(hs_length)
            tit_force.append(titin_force)
            ext_force.append(extracellular_force)
            pas_force.append(passive_force)
            act_force.append(active_force)
            tot_force.append(total_force)

    # Now plot
    ax[0].plot(hsl, tit_force, 'bo-', label='Titin')
    ax[0].plot(hsl, ext_force, 'gs-', label='Extracellular')
    ax[0].plot(hsl, pas_force, 'rd-', label='Passive')
    ax[0].plot(hsl, act_force, 'mp-', label='Active')
    ax[0].plot(hsl, tot_force, 'c+-', label='Total')
